...
$Referer_check_domain = "h2office.jp";
...
$require = array('name','email','phone','contact-type[]','message');
...
if(($confirmDsp == 0 || $sendmail == 1) && $empty_flag != 1){
	...
	if($remail == 1) {
		...
		//$re_subject = "=?iso-2022-jp?B?".base64_encode(mb_convert_encoding($re_subject,"UTF-8",$encode))."?=";
	}
	//管理者宛に届くメールをセット
	$adminBody = mailToAdmin($_POST,$subject,$mailFooterDsp,$mailSignature,$encode,$confirmDsp);
	$header = adminHeader($post_mail,$BccMail);
	//$subject = "=?iso-2022-jp?B?".base64_encode(mb_convert_encoding($subject,"UTF-8",$encode))."?=";
...
function mailToUser($arr,$dsp_name,$remail_text,$mailFooterDsp,$mailSignature,$encode){
	...
	return mb_convert_encoding($userBody,"UTF-8",$encode);
}
//必須チェック関数
